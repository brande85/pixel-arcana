...
function addDailyJournalSection(dateString) {
    ...
    let moonPhaseHTML = '';
    if (typeof isMoonPhasesEnabled !== 'undefined' && isMoonPhasesEnabled()) {
        const moonPhase = getMoonPhaseDisplay(localDate);
        moonPhaseHTML = `<div class="journal-moon-phase">${moonPhase}</div>`;
    }
...
function renderCalendar() {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        if (typeof isMoonPhasesEnabled !== 'undefined' && isMoonPhasesEnabled()) {
            ...
            const moonEmoji = getMoonPhaseEmoji(date);
            moonPhaseInfo = `<span class="calendar-moon-phase">${moonEmoji}</span>`;
        }
...
function openCardModalWithJournal(card, dateString, formattedDate) {
    ...
    if (typeof isMoonPhasesEnabled !== 'undefined' && isMoonPhasesEnabled()) {
        ...
        const moonPhase = getMoonPhaseDisplay(date);
        moonPhaseHTML = `<div class="journal-moon-phase">${moonPhase}</div>`;
    }
...
function openGameDeepDive(gameName) {
    ...
    // Get game data to check for favorite badge
    let gameData = null;
    if (typeof getGameData !== 'undefined') {
        gameData = getGameData(gameName);
    }
    
    // Set title with optional favorite badge
    modalTitle.innerHTML = gameName + (gameData?.authorFavorite ? ' <span class="author-favorite-badge" title="Author\'s Favorite">★</span>' : '');
                ...
                <div class="game-entry" id="game-${gameId}">
                    ...
                    <div class="game-entry-content">
                        <div class="game-entry-header">
                            <div class="game-title" onclick="openGameDeepDive('${game.replace(/'/g, "\\'")}'); event.stopPropagation();" style="cursor: pointer;">
                                ${game}
                                ${fullGameData?.authorFavorite ? '<span class="author-favorite-badge" title="Author\'s Favorite">★</span>' : ''}
                            </div>
...
function loadSettings() {
    ...
    // Load moon phases setting
    if (typeof isMoonPhasesEnabled !== 'undefined') {
        const moonPhasesEnabled = isMoonPhasesEnabled();
        const moonToggle = document.getElementById('moonPhasesToggle');
        if (moonToggle) {
            moonToggle.checked = moonPhasesEnabled;
        }
    }
}
...
function toggleMoonPhasesDisplay() {
    toggleMoonPhases();
    // Refresh calendar if it's the active view
    if (document.getElementById('calendar').classList.contains('active')) {
        renderCalendar();
    }
    // Refresh daily draw journal if it exists
    const journalSection = document.querySelector('.journal-section');
    if (journalSection) {
        // Re-render the daily card to update journal moon phase
        const todayString = getTodayString();
        const draws = getDailyDraws();
        if (draws[todayString]) {
            drawDailyCard();
        }
    }
}
...
function toggleGameBoyMode() {
    ...
    const isActive = body.classList.contains('gameboy-mode');
    
    if (isActive) {
        body.classList.remove('gameboy-mode');
        localStorage.setItem('gameBoyMode', 'false');
        showNotification('Normal mode restored');
    } else {
        body.classList.add('gameboy-mode');
        localStorage.setItem('gameBoyMode', 'true');
        showNotification('Game Boy mode activated! 🎮');
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    // Load moon phases state
    if (typeof isMoonPhasesEnabled !== 'undefined' && isMoonPhasesEnabled()) {
        const toggle = document.getElementById('moonPhasesToggle');
        if (toggle) toggle.checked = true;
    }
    
    // Load Game Boy mode state
    if (localStorage.getItem('gameBoyMode') === 'true') {
